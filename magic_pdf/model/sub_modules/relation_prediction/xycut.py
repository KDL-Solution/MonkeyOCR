...
def __split_projection_profile(
    arr_values: np.array,
    min_value: float,
    min_gap: float,
):
...
def _recursive_xy_cut(
    ...
    for r0, r1 in zip(arr_y0, arr_y1):
        ...
        x_projection = __projection_by_bboxes(boxes=x_sorted_boxes_chunk, axis=0)
        pos_x = __split_projection_profile(x_projection, 0, 1)
        if not pos_x:
            continue
