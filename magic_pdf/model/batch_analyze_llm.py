...
class InferenceBatch:
    ...
    def __call__(
        ...
        if self.monkeyocr.layout_model_name == MODEL_NAME.DocLayout_YOLO:
            layout_images = []
            modified_images = []
            ...
            images_layout_res += self.monkeyocr.layout_model.batch_predict(
        ...
        if self.backend in ["vllm", "lmdeploy", "openai_api"]:
            ...
            for i in range(len(images)):
                ...
                if LLMConfig.is_lora_supported(cat_ids[i]):
                    ...
                    pass
            out = self.monkeyocr.chat_model.batch_inference(new_images, messages, model_types=model_types)
            outs.extend(out)
        else:
            ### 미사용:
            # buffer = BytesIO()
